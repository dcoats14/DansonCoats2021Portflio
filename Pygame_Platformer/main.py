#Pygame Advanced Template

# KidsCanCode - Game development with Pygame video series
# Art from Kenny.nl
# Happy tune by http://opengameart.org/users/syncopiko
...
class Game(object):
    ...
    def load_data(self):
        ...
        # load sounds
        self.snds_dir = path.join(self.dir, 'snds')
        self.jump_sound = pg.mixer.Sound(path.join(self.snds_dir, 'Jump33.wav'))
        self.boost_sound = pg.mixer.Sound(path.join(self.snds_dir, 'Boost16.wav'))
    ...
    def new(self):
        ...
        for plat in PLATFORM_LIST:
            Platform(self, * plat)
        self.mob_timer = 0
        pg.mixer.music.load(path.join(self.snds_dir, 'Happy Tune.ogg'))
        for i in range(8):
            c = Cloud(self)
            c.rect.y += 500
    ...
    def show_start_screen(self):
        pg.mixer.music.load(path.join(self.snds_dir, 'Yippee.ogg'))
        pg.mixer.music.play(loops=-1)
        self.screen.fill(BGCOLOR)
